...
@Component({
  selector: 'app-admin-senha',
  imports: [ReactiveFormsModule],
  templateUrl: './admin-senha.html',
  styleUrl: './admin-senha.scss',
})
export class AdminSenha {
  ...
  protected submit(): void {
    ...
    this.adminService.alterarPropriaSenha({ senhaAtual, novaSenha }).subscribe({
      next: () => {
        this.form.reset();
        ...
        this.authService.login(email, novaSenha).subscribe({
          next: () => {
            this.saving.set(false);
            this.successMessage.set('Senha alterada com sucesso.');
            setTimeout(() => this.router.navigateByUrl('/admin/profissionais'), 1200);
          },
          error: () => {
            this.saving.set(false);
